...
typedef sxi32 (*ProcPatternMatch)(
  const char *,
  sxu32,
  const char *,
  sxu32,
...
#define SyBlobCurData( \
          BLOB)      ((void *) (&((char *) (BLOB)->pBlob)[(BLOB)->nByte]))
 ...
 */
typedef sxi32 (*ProcXMLStartTagHandler)(
  SyXMLRawStr *,
  SyXMLRawStr *,
  sxu32,
  SyXMLRawStr *,
...
typedef sxi32 (*ProcXMLSyntaxErrorHandler)(
  const char *, int, SyToken *,
  void *
);
...
PH7_PRIVATE sxi32       PH7_MemObjAdd(
  ph7_value *pObj1,
  ph7_value *pObj2,
  int bAddStore
);
PH7_PRIVATE sxi32 PH7_MemObjCmp(
  ph7_value *pObj1,
  ph7_value *pObj2,
  int bStrict,
  int iNest
);
PH7_PRIVATE sxi32 PH7_MemObjInitFromString(
  ph7_vm *pVm,
  ph7_value *pObj,
  const SyString *pVal
);
PH7_PRIVATE sxi32 PH7_MemObjInitFromArray(
  ph7_vm *pVm,
  ph7_value *pObj,
  ph7_hashmap *pArray
);
...
PH7_PRIVATE sxi32 PH7_MemObjInitFromReal(
  ph7_vm *pVm,
  ph7_value *pObj,
  ph7_real rVal
);
...
#endif
PH7_PRIVATE sxi32 PH7_MemObjInitFromInt(
  ph7_vm *pVm, ph7_value *pObj,
  sxi64 iVal
);
PH7_PRIVATE sxi32 PH7_MemObjInitFromBool(
  ph7_vm *pVm,
  ph7_value *pObj,
  sxi32 iVal
);
PH7_PRIVATE sxi32 PH7_MemObjInit(ph7_vm *pVm, ph7_value *pObj);
PH7_PRIVATE sxi32 PH7_MemObjStringAppend(
  ph7_value *pObj,
  const char *zData,
  sxu32 nLen
);
...
#if 0
/* Not used in the current release of the PH7 engine */
PH7_PRIVATE sxi32 PH7_MemObjStringFormat(
  ph7_value *pObj,
  const char *zFormat,
  va_list ap
);
...
PH7_PRIVATE sxi32 PH7_TokenizeRawText(
  const char *zInput,
  sxu32 nLen,
  SySet *pOut
);
PH7_PRIVATE sxi32 PH7_TokenizePHP(
  const char *zInput,
  sxu32 nLen,
  sxu32 nLineStart,
  SySet *pOut
...
PH7_PRIVATE sxi32 PH7_VmInitFuncState(
  ph7_vm *pVm, ph7_vm_func *pFunc, const char *zName, sxu32 nByte,
...
PH7_PRIVATE sxi32 PH7_VmCallClassMethod(
  ph7_vm *pVm, ph7_class_instance *pThis, ph7_class_method *pMethod,
...
PH7_PRIVATE sxi32 PH7_VmUnsetMemObj(
  ph7_vm *pVm, sxu32 nObjIdx,
  int bForce
);
PH7_PRIVATE void       PH7_VmRandomString(ph7_vm *pVm, char *zBuf, int nLen);
PH7_PRIVATE ph7_class* PH7_VmPeekTopClass(ph7_vm *pVm);
PH7_PRIVATE int PH7_VmIsCallable(
  ph7_vm *pVm, ph7_value *pValue,
  int CallInvoke
);
...
PH7_PRIVATE sxi32 PH7_GetNextExpr(
  SyToken *pStart,
  SyToken *pEnd,
  SyToken **ppNext
);
...
PH7_PRIVATE const ph7_expr_op* PH7_ExprExtractOperator(
  SyString *pStr,
  SyToken *pLast
);
...
PH7_PRIVATE sxi32 PH7_CompileLangConstruct(
  ph7_gen_state *pGen,
  sxi32 iCompileFlag
);
...
PH7_PRIVATE sxi32 PH7_CompileSimpleString(
  ph7_gen_state *pGen,
  sxi32 iCompileFlag
);
...
PH7_PRIVATE void PH7_HashmapExtractNodeKey(
  ph7_hashmap_node *pNode,
  ph7_value *pKey
);
...
PH7_PRIVATE sxi32 PH7_InputFormat(
  int (*xConsumer)(ph7_context *, const char *, int, void *),
  ph7_context *pCtx,
  const char *zIn,
  int nByte,
  int nArg,
...
PH7_PRIVATE sxi32 PH7_ProcessCsv(
  const char *zInput, int nByte, int delim, int encl,
...
PH7_PRIVATE ph7_class* PH7_NewRawClass(
  ph7_vm *pVm,
  const SyString *pName,
  sxu32 nLine
);
...
PH7_PRIVATE sxi32 PH7_ClassInstallAttr(
  ph7_class *pClass,
  ph7_class_attr *pAttr
);
PH7_PRIVATE sxi32 PH7_ClassInstallMethod(
  ph7_class *pClass,
  ph7_class_method *pMeth
);
PH7_PRIVATE sxi32 PH7_ClassInherit(
  ph7_gen_state *pGen,
  ph7_class *pSub,
  ph7_class *pBase
);
...
PH7_PRIVATE sxi32 SyXMLProcess(
  SyXMLParser *pParser,
  const char *zInput,
  sxu32 nByte
);
...
PH7_PRIVATE sxi32 SyArchiveGetNextEntry(
  SyArchive *pArch,
  SyArchiveEntry **ppEntry
);
PH7_PRIVATE sxi32 SyZipExtractFromBuf(
  SyArchive *pArch,
  const char *zBuf,
  sxu32 nLen
);
...
PH7_PRIVATE sxi32 SyMD5Compute(const void *pIn, sxu32 nLen,
                               unsigned char zDigest[16]);
PH7_PRIVATE void SHA1Init(SHA1Context *context);
PH7_PRIVATE void SHA1Update(
  SHA1Context *context,
...
PH7_PRIVATE sxi32 SySha1Compute(const void *pIn, sxu32 nLen,
...
PH7_PRIVATE sxu32 SyBufferFormat(
  char *zBuf,
  sxu32 nLen,
  const char *zFormat,
  ...
);
PH7_PRIVATE sxu32 SyBlobFormatAp(
  SyBlob *pBlob, const char *zFormat,
  va_list ap
);
PH7_PRIVATE sxu32 SyBlobFormat(SyBlob *pBlob, const char *zFormat, ...);
PH7_PRIVATE sxi32 SyProcFormat(
  ProcConsumer xConsumer,
  void *pData,
  const char *zFormat,
  ...
...
PH7_PRIVATE char* SyMemBackendStrDup(
...
PH7_PRIVATE sxi32 SyMemBackendInitFromParent(
  SyMemBackend *pBackend,
...
#if 0
/* Not used in the current release of the PH7 engine */
PH7_PRIVATE void* SyMemBackendPoolRealloc(
  SyMemBackend *pBackend,
  void *pOld,
  sxu32 nByte
);
...
PH7_PRIVATE void* SyMemBackendRealloc(
  SyMemBackend *pBackend,
  void *pOld,
  sxu32 nByte
);
...
#if defined(PH7_ENABLE_THREADS)
PH7_PRIVATE sxi32 SyMemBackendMakeThreadSafe(
  SyMemBackend *pBackend,
...
#endif
PH7_PRIVATE sxi32 SyByteListFind(
  const char *zSrc,
  sxu32 nLen,
  const char *zList,
  sxu32 *pFirstPos
...
#ifndef PH7_DISABLE_BUILTIN_FUNC
PH7_PRIVATE sxi32 SyByteFind2(
  const char *zStr, sxu32 nLen, sxi32 c,
  sxu32 *pPos
);

#endif
PH7_PRIVATE sxi32 SyByteFind(
  const char *zStr, sxu32 nLen, sxi32 c,
  sxu32 *pPos
);
