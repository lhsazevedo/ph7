...
PH7_NewRawClass(ph7_vm *pVm, const SyString *pName, sxu32 nLine)
{
  ph7_class *pClass;
  char *zName;
  /* Allocate a new instance */
  pClass = (ph7_class *) SyMemBackendPoolAlloc(
    &pVm->sAllocator,
    sizeof(ph7_class)
  );
  if (pClass == 0) {
...
)
{
  ...
  char *zName;
  pAttr = (ph7_class_attr *) SyMemBackendPoolAlloc(
    &pVm->sAllocator,
 ...
 * random unique name.
 */
PH7_PRIVATE ph7_class_method *
PH7_NewClassMethod(
  ph7_vm *pVm, ph7_class *pClass, const SyString *pName, sxu32 nLine,
...
)
{
  ...
  pMeth = (ph7_class_method *) SyMemBackendPoolAlloc(
    &pVm->sAllocator,
    sizeof(ph7_class_method)
  );
  if (pMeth == 0) {
    return 0;
  }
  ...
  if (pEntry == 0) {
    ...
    nByte = sizeof(zSalt) + pName->nByte + SyStringLength(&pClass->sName)
            + sizeof(char) * 7 /*[[__'\0'*/;
  ...
  if (iProtection != PH7_CLASS_PROT_PUBLIC) {
    if ((pName->nByte == sizeof("__construct") - 1 && SyMemcmp(
      pName->zString,
      "__construct",
      sizeof("__construct") - 1
         ) == 0)
        || (pName->nByte == sizeof("__destruct") - 1 && SyMemcmp(
          pName->zString,
          "__destruct",
          sizeof("__destruct") - 1
            ) == 0)
  ...
  PH7_VmInitFuncState(
    &(*pVm), &pMeth->sFunc,
    &zName[sizeof(char) * 4 /*[__@*/ + SyStringLength(&pClass->sName)],
 ...
 */
PH7_PRIVATE ph7_class_method *
PH7_ClassExtractMethod(ph7_class *pClass, const char *zName, sxu32 nByte)
{
  SyHashEntry *pEntry;
 ...
 */
PH7_PRIVATE ph7_class_attr *
PH7_ClassExtractAttribute(ph7_class *pClass, const char *zName, sxu32 nByte)
{
  SyHashEntry *pEntry;
  /* Perform a hash lookup */
 ...
 */
PH7_PRIVATE sxi32
PH7_ClassInstallAttr(ph7_class *pClass, ph7_class_attr *pAttr)
{
 ...
 */
PH7_PRIVATE sxi32
PH7_ClassInstallMethod(ph7_class *pClass, ph7_class_method *pMeth)
{
 ...
 */
PH7_PRIVATE sxi32
PH7_ClassInherit(ph7_gen_state *pGen, ph7_class *pSub, ph7_class *pBase)
{
  ph7_class_method *pMeth;
  ph7_class_attr *pAttr;
  SyHashEntry *pEntry;
  ...
  rc = SyHashInsert(
    &pBase->hDerived,
    (const void *) SyStringData(&pSub->sName),
    SyStringLength(&pSub->sName),
    pSub
  );
  if (rc != SXRET_OK) {
    return rc;
  ...
  while ((pEntry = SyHashGetNextEntry(&pBase->hAttr)) != 0) {
                  ...
                  )) != 0)
    {
      if (pAttr->iProtection == PH7_CLASS_PROT_PRIVATE
          && ((ph7_class_attr *) pEntry->pUserData)->iProtection
          != PH7_CLASS_PROT_PUBLIC)
      {
        /* Cannot redeclare private attribute */
        PH7_GenCompileError(
          &(*pGen), E_WARNING, ((ph7_class_attr *) pEntry->pUserData)->nLine,
                  ...
                  )) != 0)
    {
      if (pMeth->iFlags & PH7_CLASS_ATTR_FINAL) {
        ...
        rc = PH7_GenCompileError(
          &(*pGen), E_ERROR, ((ph7_class_method *) pEntry->pUserData)->nLine,
 ...
 */
PH7_PRIVATE sxi32
PH7_ClassInterfaceInherit(ph7_class *pSub, ph7_class *pBase)
{
  ph7_class_method *pMeth;
  ph7_class_attr *pAttr;
  SyHashEntry *pEntry;
  ...
  while ((pEntry = SyHashGetNextEntry(&pBase->hAttr)) != 0) {
    ...
    pName = &pAttr->sName;
    if (SyHashGet(
      &pSub->hAttr, (const void *) pName->zString,
      pName->nByte
        ) == 0)
    {
  ...
  while ((pEntry = SyHashGetNextEntry(&pBase->hMethod)) != 0) {
    ...
    pName = &pMeth->sFunc.sName;
    if (SyHashGet(
      &pSub->hMethod, (const void *) pName->zString,
      pName->nByte
        ) == 0)
    {
      /* Install the method */
 ...
 */
PH7_PRIVATE sxi32
PH7_ClassImplement(ph7_class *pMain, ph7_class *pInterface)
{
  ph7_class_attr *pAttr;
  SyHashEntry *pEntry;
  SyString *pName;
 ...
 */
static ph7_class_instance *
NewClassInstance(ph7_vm *pVm, ph7_class *pClass)
{
  ph7_class_instance *pThis;
  /* Allocate a new instance */
  pThis = (ph7_class_instance *) SyMemBackendPoolAlloc(
    &pVm->sAllocator,
    sizeof(ph7_class_instance)
  );
  if (pThis == 0) {
    return 0;
  }
 ...
 */
PH7_PRIVATE ph7_class_instance *
PH7_NewClassInstance(ph7_vm *pVm, ph7_class *pClass)
{
 ...
 */
static ph7_value *
ExtractClassAttrValue(ph7_vm *pVm, VmClassAttr *pAttr)
{
  /* Extract the value */
  ph7_value *pValue;
 ...
 */
PH7_PRIVATE ph7_class_instance *
PH7_CloneClassInstance(ph7_class_instance *pSrc)
{
  ...
  while ((pEntry = SyHashGetNextEntry(&pSrc->hAttr)) != 0
         && (pEntry2 = SyHashGetNextEntry(&pClone->hAttr)) != 0)
  {
    ...
    if ((pSrcAttr->pAttr->iFlags
         & (PH7_CLASS_ATTR_STATIC | PH7_CLASS_ATTR_CONSTANT)) == 0)
    {
  ...
  if (pMethod) {
    if (pMethod->iCloneDepth < 16) {
      ...
      PH7_VmThrowError(
        ...
        0,
        PH7_CTX_ERR,
        "Object clone limit reached,no more call to __clone()"
      );
    }
    /* Reset the cursor */
 ...
 */
static void
PH7_ClassInstanceRelease(ph7_class_instance *pThis)
{
 ...
 */
PH7_PRIVATE void
PH7_ClassInstanceUnref(ph7_class_instance *pThis)
{
  pThis->iRef--;
  if (pThis->iRef < 1) {
...
PH7_ClassInstanceExtractAttrValue(ph7_class_instance *pThis, VmClassAttr *pAttr)
{
 ...
 */
PH7_PRIVATE sxi32
PH7_ClassInstanceToHashmap(ph7_class_instance *pThis, ph7_hashmap *pMap)
{
  SyHashEntry *pEntry;
  SyString *pAttrName;
  VmClassAttr *pAttr;
 ...
 */
PH7_PRIVATE ph7_value *
PH7_ClassInstanceFetchAttr(ph7_class_instance *pThis, const SyString *pName)
{
  SyHashEntry *pEntry;
  VmClassAttr *pAttr;
  /* Query the attribute hashtable */
  pEntry = SyHashGet(
    &pThis->hAttr, (const void *) pName->zString,
    pName->nByte
  );
  if (pEntry == 0) {
    /* No such attribute */
  ...
  if (pAttr->pAttr->iFlags
      & (PH7_CLASS_ATTR_CONSTANT | PH7_CLASS_ATTR_STATIC))
  {
    /* Access is forbidden */
    return 0;
  }
